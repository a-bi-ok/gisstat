...
public class GeologyTable extends GISSTATDBase {
    
    private static String tableName = "refgeology";
    // jdbc Connection
    private static Connection conn = null;
	private static List<String> lithoTypes = new ArrayList<String>();
...
//	rs = s.executeQuery(
// 		   "select fold, fault, rocktype\n" + 
// 		   "           from refstructure, regeology\n" + 
// 		   "           where refstructure.jid = regeology.jid;");
...
//	             We create a table...
	            s.execute("create table refgeology "
	            		+ "(rid int not null,"
...
//            statements.add(s);
            rs = s.executeQuery(
       		   "select distinct rocktype from refgeology");
            ResultSetMetaData rsmd = rs.getMetaData();
            System.out.println(rsmd.getColumnLabel(1));       
//            System.out.println("------------");
            while (rs.next()) {
         	   lithoTypes.add(rs.getString(1));
...
//    }
    public void dropTable(String tableName) {
        try {
            ...
            conn.setAutoCommit(false);
            s = conn.createStatement();
            s.execute("drop table refgeology");
      		System.out.println("Dropped table 'refgeology'");
            conn.commit();
            System.out.println("Committed the transaction");
